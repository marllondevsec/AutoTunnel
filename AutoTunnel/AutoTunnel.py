...
<body>
    ...
    <p>{datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>
        ...
        # Save process info for HTTP server (type: server)
        save_process_info("http_server", os.getpid(), port, 
...
def start_tunnel_flow(only_tunnel=False):
    ...
    # Start tunnel and save process info
    plugin.start(port)
    ...
    for i in range(40):
        ...
        if url:
            ...
            if hasattr(plugin, 'pid') and plugin.pid:
                # Find and update the process info with the URL
                active_file = DATA_DIR / "active_processes.json"
                if active_file.exists():
                    active = json.loads(active_file.read_text(encoding="utf-8"))
...
def show_active_urls():
    """Show ACTIVE tunnels and servers with management options"""
    # Clean up dead processes first
    cleanup_dead_processes()
    ...
    while True:
        ...
        if not active:
            ...
            # Still show saved URLs if no active processes
            ...
            if console:
                ...
                for i, process in enumerate(active, 1):
                    name = process.get("name", "Unknown")
                    process_type = process.get("type", "tunnel")
                    port = process.get("port", "?")
                    url = process.get("url", tr("no_url_available"))
                    ...
                    table.add_row(
                        str(i),
                        name,
                        tr(f"process_type_{process_type}") if tr(f"process_type_{process_type}") else process_type,
                        str(port),
                        url
                    )
                ...
                for i, process in enumerate(active, 1):
                    ...
                    port = process.get("port", "?")
                    url = process.get("url", tr("no_url_available"))
                    print(f"{i}) {name} [{process_type}] - Port: {port} - {url}")
        ...
        if sel == 1 and active:
            ...
            if len(active) == 1:
                ...
                # Multiple processes - choose which to stop
                print_header()
                cprint(f"[bold]🛑 {tr('stop_process')}[/bold]", "cyan")
                ...
                for i, process in enumerate(active, 1):
                    ...
                    pid = process.get("pid", "?")
                    port = process.get("port", "?")
                    stop_opts.append(f"{name} (PID: {pid}, Port: {port})")
                
                stop_opts.append(f"⬅️ {tr('back')}")
                
                stop_sel = numeric_choice(tr("choose_process_to_stop"), stop_opts)
                
                if stop_sel is None or stop_sel > len(active):
                    continue
                
                process = active[stop_sel - 1]
                pid = process["pid"]
                name = process["name"]
        ...
        elif (sel == 2 and active) or (sel == 1 and not active):
            # Copy URL
            if not active:
                # No active processes, try to copy saved URLs
                urls_to_copy = []
                ...
                if len(urls_to_copy) == 1:
                    ...
                    # Multiple saved URLs to choose from
                    print_header()
                ...
                if len(active) == 1:
                    ...
                    # Multiple active processes - choose which URL to copy
                    print_header()
                    cprint(f"[bold]📋 {tr('copy_url')}[/bold]", "cyan")
                    ...
                    for i, process in enumerate(active, 1):
                        ...
                        copy_opts.append(f"{name}: {display_url}")
                    
                    copy_opts.append(f"⬅️ {tr('back')}")
                    
                    copy_sel = numeric_choice(tr("choose_url_to_copy"), copy_opts)
                    
                    if copy_sel is None or copy_sel > len(active):
                        continue
                    
                    process = active[copy_sel - 1]
                    url = process.get("url")
                    if url:
                        try:
                            import pyperclip
...
def show_status():
    ...
    for idx, (name, plugin) in PLUGINS.items():
        ...
        # Check if this plugin is running by looking at active processes
        is_running = False
        ...
        # If running, show details from active process
        if is_running:
            for process in active_tunnels:
                if process.get("name") == name:
    ...
    print("\n" + "="*60)
    cprint(f"📊 {tr('summary', running_tunnels)}", "green" if running_tunnels > 0 else "yellow")
    
    # Show all active processes
    if active:
        print("\n" + "="*60)
        cprint(f"🔗 {tr('active_processes')}:", "cyan")
        ...
        for process in active:
            ...
            type_display = tr(f"process_type_{process_type}") if tr(f"process_type_{process_type}") else process_type
            
            cprint(f"  {name} [{type_display}] - PID: {pid}, Port: {port}", "white")
            if url and url != tr("no_url_available"):
                cprint(f"     URL: {url}", "green")
